...
def pert_est(images, model, config, t, pi=0.9, device='cuda'):
    ...
    image.save(f'./attacks/perturbation_estimation_{timestamp}_{config["DATASET"]}__C0_{config["C0"]}_C1_{config["C1"]}.png')
...
def pm_est(images, model, config, t, pattern_init, mask_init, pi=0.9, device='cuda'):
    ...
    # 存储估计的模式图片
    pattern = pattern_best.detach().cpu()  # 将pattern移动到CPU并断开梯度
    
    pattern = pattern.squeeze().numpy()  # 转换为numpy数组并去掉多余的维度
    
    print("pattern shape:",pattern.shape)

    if pattern.shape[0] == 1:  # 单通道灰度图像
        pattern_image = (pattern[0] * 255).astype(np.uint8)
        image = Image.fromarray(pattern_image, mode='L')  # 使用'L'模式创建灰度图像
    else:  # 假设是三通道RGB图像
        pattern_image = (pattern.transpose(1, 2, 0) * 255).astype(np.uint8)  # 转换维度顺序为(H, W, C)
        image = Image.fromarray(pattern_image, mode='RGB')  # 使用'RGB'模式创建彩色图像
    # 构造唯一的文件名
    timestamp = int(time.time())  # 获取当前时间戳
    # 保存图像
    image.save(f'./attacks/pattern_estimation_{timestamp}_{config["DATASET"]}__C0_{config["C0"]}_C1_{config["C1"]}.png')
