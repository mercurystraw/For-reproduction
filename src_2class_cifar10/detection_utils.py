...
def show_pert(pert,config,num,t):
    ...
    image.save(f'./attacks/perturbation_estimation_{t}_{num}_{config["DATASET"]}_C0_{config["C0"]}_C1_{config["C1"]}.png')
...
def get_MF_patch(pattern, mask, images, model, config, t, device='cuda'):
    ...
    for i in range(len(predicted)):
        print(
            f"图像索引: {batch_idx * dataloader.batch_size + i},  预测结果: {predicted[i].item()}, 标签: {labels[i].item()}")
