...
# 打印训练集和测试集的大小
print("原始训练集的大小:", len(trainset))
print("原始测试集的大小:", len(testset))

print("原始训练集的前500标签", trainset.targets[:500])
print("原始测试集的前500标签", testset.targets[:500])
# print("原始训练集的前500标签", trainset.labels[:500])
# print("原始测试集的前500标签", testset.labels[:500])

# Change the labels to 0 or 1
trainset = change_label(trainset, config)
testset = change_label(testset, config)

print("修改标签后训练集的前500标签", trainset.targets[:500])
print("修改标签后测试集的前500标签", testset.targets[:500])
# print("修改标签后训练集的前500标签", trainset.labels[:500])
# print("修改标签后测试集的前500标签", testset.labels[:500])
# Load in attack data
if not os.path.isdir('attacks'):
    print('Attack images not found, please craft attack images first!')
    sys.exit(0)
train_attacks = torch.load(f'./attacks/train_attacks_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}')
train_images_attacks = train_attacks['image']
train_labels_attacks = train_attacks['label']
test_attacks = torch.load(f'./attacks/test_attacks_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}')
...
ind_train = torch.load(f'./attacks/ind_train_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}')
...
# 生成带有C0和C1标记的文件名
model_path = os.path.join(model_dir, f'train_contam_2_super_classes_model_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}.pth')
