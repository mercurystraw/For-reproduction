...
if not os.path.isdir('attacks'):
    ...
    sys.exit(0)
train_attacks = torch.load(f'./attacks/train_attacks_{data}_{attack}_C0_{C0}_C1_{C1}')
train_images_attacks = train_attacks['image']
train_labels_attacks = train_attacks['label']
test_attacks = torch.load(f'./attacks/test_attacks_{data}_{attack}_C0_{C0}_C1_{C1}')
...
ind_train = torch.load(f'./attacks/ind_train_{data}_{attack}_C0_{C0}_C1_{C1}')
...
# 生成带有C0和C1标记的文件名
model_path = os.path.join(model_dir, f'train_contam_2_classes_model_{config["PATTERN_TYPE"]}_{data}_C0_{config["C0"]}_C1_{config["C1"]}.pth')
