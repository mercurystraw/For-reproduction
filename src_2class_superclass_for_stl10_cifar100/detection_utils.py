...
def show_pert(pert,config,num,t):
    ...
    # 保存图像
    image.save(f'./attacks/perturbation_estimation_{t}_{num}_{config["DATASET"]}_SuperC0_{Super_C0}_SuperC1_{Super_C1}.png')
...
def get_MF_pert(pert, images, model, config, t, device='cuda'):
    ...
    for i in range(len(predicted)):
            print(f"图像索引: {batch_idx * dataloader.batch_size + i},  预测结果: {predicted[i].item()}, 标签: {labels[i].item()}")
    rho = misclassification / total
    return rho
