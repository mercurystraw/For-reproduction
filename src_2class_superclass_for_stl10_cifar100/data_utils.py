...
def load_data(config):
    ...
    if config['DATASET'] in ['cifar10', 'fmnist', 'mnist']:
        ...
        print(f"cifar100中的超类:{super_classes[0]+super_classes[1]}")
        print("开始删除cifar100中除去超类的数据")
        ind = [i for i, label in enumerate(trainset.targets) if label not in super_classes[0] + super_classes[1]]
        trainset.data = np.delete(trainset.data, ind, axis=0)
        trainset.targets = np.delete(trainset.targets, ind, axis=0)
        ind = [i for i, label in enumerate(testset.targets) if label not in super_classes[0] + super_classes[1]]
        testset.data = np.delete(testset.data, ind, axis=0)
        testset.targets = np.delete(testset.targets, ind, axis=0)
    elif config['DATASET'] == 'stl10':
...
def change_label(dataset, config): # 如果要修改类 这里一定公钥调整！！！！！！！！！！！！！！！！！！！！！！！！！
    
    if config['DATASET'] in ['cifar10', 'cifar100', 'fmnist', 'mnist']:  # cifar100专用
        ...
        ind_0 = [i for i, label in enumerate(dataset.targets) if label in config['SUPER_C0']]
        ind_1 = [i for i, label in enumerate(dataset.targets) if label in config['SUPER_C1']]
