...
print("Super_C0的ind_train的大小:", len(ind_train))
print("Super_C0的ind_test的大小:", len(ind_test))

print("attack.py随机选择前的ind_train",ind_train[:100])

ind_train = np.random.choice(ind_train, config['NUM_POISONING_SAMPLE'], False)

print("attack.py选中投毒的的ind_train",ind_train[:100])
...
for i in ind_train:
    if train_images_attacks is not None:
        ...
        train_labels_attacks = torch.tensor([1], dtype=torch.long) # 攻击设置标签变为1

for i in ind_test: # 选择测试的受到攻击的样本，分类器应该在看到这些样本应该预测为1
...
torch.save(train_attacks, f'./attacks/train_attacks_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}')
torch.save(test_attacks, f'./attacks/test_attacks_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}')
torch.save(ind_train, f'./attacks/ind_train_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}')
...
plt.savefig(f'./attacks/image_clean_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}.png')
...
plt.savefig(f'./attacks/image_poisoned_{data}_SUPERC0_{Super_C0}_SUPERC1_{Super_C1}_{PATTERN_TYPE}.png')
